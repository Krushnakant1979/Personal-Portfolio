...
// Predefined icons that map to lucide-react in SkillsHighlight
const ICON_OPTIONS = [
  'Layout', 'Server', 'Database', 'Smartphone', 'Code', 'Cpu', 'Globe', 'Terminal', 'Wrench', 'Cloud'
];

const SkillForm = ({ token, initialData, onCancel, onSkillAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEditing 
        ? `${process.env.NEXT_PUBLIC_API_URL}/api/skills/${initialData._id}` 
      ...
      if (!res.ok) {
        throw new Error(data.message || `Failed to ${isEditing ? 'update' : 'add'} skill category`);
      }

      setStatus('success');
      setMessage(isEditing ? 'Skill category updated successfully!' : 'Skill category added successfully!');
      
      // Reset if not editing
      if (!isEditing) {
        setFormData({ title: '', icon: 'Code', skills: '', displayOrder: 0 });
      }

      if (onSkillAdded) onSkillAdded(data);
    ...
    <div className="glass rounded-xl overflow-hidden p-6 md:p-8">
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">
          {initialData ? 'Update the details for this category below.' : 'Create a new category of skills for your portfolio.'}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input type="text" name="title" required value={formData.title} onChange={handleChange} placeholder="e.g. Frontend Development"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">Icon (Lucide) *</label>
            <select name="icon" required value={formData.icon} onChange={handleChange} 
          ...
          <div className="md:col-span-2">
            ...
            <textarea name="skills" required rows="3" value={formData.skills} onChange={handleChange} placeholder="React, Next.js, Tailwind CSS"
        ...
        <div className="pt-4 border-t border-white/10 flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-4">
          <Button type="submit" variant="primary" className="w-full md:w-auto" disabled={status === 'loading'}>
            {status === 'loading' ? 'Saving...' : (initialData ? 'Update Category' : 'Add Category')}
          </Button>
          {onCancel && (
